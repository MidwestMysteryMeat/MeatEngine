...
// long enough to be its own denial of service.
inline constexpr std::size_t kMaxNameBytes = 64;
inline constexpr std::size_t kMaxTokenBytes = 128;
inline constexpr std::size_t kMaxAssetPathBytes = 256;

struct HelloMsg {
    std::uint16_t protocol = kProtocolVersion;
    std::string name;
    // Proof that this peer is the session owner's own client. Generated by the
    // server at boot and handed to that client in-process, so it never crosses
