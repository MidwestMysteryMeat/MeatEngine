...
uniform int uShadows; // 0/1 — A2 sun shadow map
...
// A2: PCF shadow factor for the directional sun (1 = lit, ~0.35 = soft PSX umbra).
...
vec3 shadeDirect(vec3 albedo, vec3 n, vec3 v, vec3 worldPos) {
    ...
    vec3 c = blinnPhong(albedo, n, v, normalize(-uDirLightDir.xyz), uDirLightColor.rgb, 1.0) * sh;
